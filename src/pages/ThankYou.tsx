...
type Status = "checking" | "verified" | "failed" | "invalid";
...
export default function ThankYou() {
  ...
  // Quick client-side format guard — catches direct URL access with no params at all
  const paramsLookValid =
    paymentId.startsWith("pay_") &&
    linkId.startsWith("plink_") &&
    linkStatus === "paid" &&
    signature.length > 10;

  useEffect(() => {
    if (!paramsLookValid) {
      // No Razorpay params at all — someone typed the URL → redirect home
    ...
    const check = async () => {
      ...
      try {
        ...
        if (res.status === 400 || res.status === 403) {
          // Bad params — not a real payment
          if (!cancelled) setStatus("failed");
          return;
        }

        // 202 = webhook not arrived yet, retry
  ...
  }, [paramsLookValid, paymentId, linkId, linkStatus, navigate]);

  // ── Checking ────────────────────────────────────────────────────────────
  if (status === "checking") {
    return (
      <div className="min-h-screen bg-luxury-gradient flex flex-col items-center justify-center p-6 text-gold-900 font-serif">
        <div className="max-w-md w-full bg-white/90 backdrop-blur-sm p-8 rounded-2xl border border-gold-400/30 shadow-2xl flex flex-col items-center text-center space-y-6">
          ...
          <h1 className="text-2xl font-bold text-gold-700">Confirming your payment…</h1>
          <p className="text-sm text-gold-900/70 font-light">Please wait while we securely verify your transaction.</p>
      ...
      <div className="min-h-screen bg-luxury-gradient flex flex-col items-center justify-center p-6 text-gold-900 font-serif">
        <div className="max-w-md w-full bg-white/90 backdrop-blur-sm p-8 rounded-2xl border border-gold-400/30 shadow-2xl flex flex-col items-center text-center space-y-5">
          ...
          <p className="text-sm text-gold-900/70 font-light leading-relaxed">
            We could not confirm your payment. The webhook may still be processing — please wait a moment and refresh.
          </p>
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="px-6 py-2.5 border border-gold-400/40 text-gold-800 rounded-full text-sm hover:bg-gold-50 transition-colors"
            >
              Retry
            </button>
    ...
    <div className="min-h-screen bg-luxury-gradient flex flex-col items-center justify-center p-6 sm:p-12 text-gold-900 font-serif">
      <div className="max-w-3xl w-full bg-white/90 backdrop-blur-sm p-8 sm:p-12 rounded-2xl border border-gold-400/30 shadow-2xl flex flex-col items-center text-center space-y-6">
        ...
        <div className="text-left w-full space-y-4 text-sm sm:text-base text-gold-900/70 bg-gold-50/30 p-6 rounded-xl border border-gold-400/20">
          ...
          <div className="space-y-4">
            <p>
              ...
              to fill out the short application. Be very honest with your answers as per your current situation.
            </p>
            <p>
              2) Please check your email — we have already sent you the application form. If it went to spam, kindly move it to Primary.
            </p>
            <p>
        ...
        <p className="text-sm text-gold-900/60 max-w-2xl font-light">
          If we are not able to take your case, a refund against the booking charges will be issued within 7 days.
        </p>
        ...
        <Link to="/" className="mt-4 px-8 py-3 bg-gold-500 hover:bg-gold-600 text-white rounded-full text-sm font-semibold tracking-wider border border-gold-400 transition-colors">
          Return to Home
        </Link>
